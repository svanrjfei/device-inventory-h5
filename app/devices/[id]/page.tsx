...
export default function DeviceDetailPage() {
  ...
  function toggleMissing() {
    ...
    toast.promise(
      devicesApi.patch(d.id, { missing: true }).then((res) => {
        setD(res);
        return res;
      }),
    ...
    <div className="mx-auto max-w-xl px-0">
      ...
      <div className="px-4 -mt-1 pb-2 flex items-center justify-end gap-2">
        <Button variant="outline" size="sm" onClick={toggleMissing}>
          <AlertTriangle
            size={16}
            className={d.missing ? "text-red-500" : "text-neutral-400"}
          />
      ...
      <div className="mx-4 space-y-3">
        <div className="rounded-lg border bg-white shadow-sm">
          <div className="px-4 py-3 border-b bg-gradient-to-r from-neutral-50 to-white flex items-center justify-between">
            ...
            <span className="text-[11px] rounded-full px-2 py-0.5 border border-neutral-300 text-neutral-700 bg-neutral-50">
              {d.status}
            </span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
              <div className="flex items-center text-neutral-700">
                <Tag size={16} className="mr-1.5 text-neutral-400" />
                类型：{d.deviceType}
              </div>
              <div className="flex items-center text-neutral-700">
                单位：{d.unit ?? "无"}
              ...
              <div className="flex items-center text-neutral-700">
                <DollarSign size={16} className="mr-1.5 text-neutral-400" />
                单价：{formatMoney(d.unitPrice)}
              </div>
              <div className="flex items-center text-neutral-700">
                数量：{d.quantity ?? "无"}
              </div>
              <div className="flex items-center text-neutral-700">
                总价：{formatMoney(d.totalPrice)}
              </div>
              <div className="flex items-center text-neutral-700">
                部门：{d.department ?? "无"}
              </div>
              ...
              <div className="flex items-center text-neutral-700">
                ...
                入库：{formatDate(d.storageAt)}
              </div>
              <div className="flex items-center text-neutral-700">
                用途：{d.usage ?? "无"}
              </div>
              <div className="flex items-center text-neutral-700">
                出厂编号：{d.factoryNumber ?? "无"}
              </div>
              <div className="flex items中心 text-neutral-700">
                发票号：{d.invoiceNumber ?? "无"}
              </div>
              <div className="flex items-center text-neutral-700">
                经费编号：{d.fundingCode ?? "无"}
              </div>
              <div className="flex items-center text-neutral-700">
                经费来源：{d.funding ?? "无"}
              </div>
